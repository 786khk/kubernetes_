...
kubectl config set-context {my namespace}@{custom context name} --cluster=kubenetes --user=kubenetes-admin --namespace={my namespaces}
kubectl config view
kubectl config use-context [my namespace@custom context name]

```

- my namespace@custom context name의 컨텍스트 생성, 클러스터는 kubenetes,사용자는 쿠버네티스 어드민권한을 가지며 기본 네임스페이스는 mynamespace
- context 생성한 컨텍스트 확인
- 생성한 컨텍스트로 사용
